...
import { Activity, Mail, Lock, User, Briefcase, ArrowRight, Eye, EyeOff } from 'lucide-react'
...
export function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await authApi.register({ name, email, password, org })
      ...
      >
        ...
        <div className="relative z-10 text-center">
          ...
          <p className="mt-3 text-sm text-slate-400">
            Join MarketTrace and secure your enterprise
          </p>
        </div>
        
        <form className="relative z-10 mt-6 space-y-4" onSubmit={handleRegister}>
          {error && (
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {password.length > 0 && (
                <div className="mt-3 flex gap-1 h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                  <div className={`h-full flex-1 ${password.length > 0 ? 'bg-red-400' : ''}`} />
                  <div className={`h-full flex-1 ${password.length > 5 ? 'bg-amber-400' : ''}`} />
                  <div className={`h-full flex-1 ${password.length > 8 ? 'bg-green-500' : ''}`} />
                </div>
              )}
            </div>
          </div>
