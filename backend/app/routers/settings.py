...
async def get_settings(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    groq_key = await _get_setting(db, "groq_api_key", current_user.id)
    anthropic_key = await _get_setting(db, "anthropic_api_key", current_user.id)
    return SettingsResponse(
        groq_api_key_set=bool(groq_key),
        anthropic_api_key_set=bool(anthropic_key),
        default_profile_id=await _get_setting(db, "default_profile_id", current_user.id),
        replay_speed=int(await _get_setting(db, "replay_speed", current_user.id) or "1"),
        theme=await _get_setting(db, "theme", current_user.id) or "light",
        slack_bot_key_set=bool(await _get_setting(db, "slack_bot_key", current_user.id)),
        slack_channel=await _get_setting(db, "slack_channel", current_user.id),
        atlassian_endpoint=await _get_setting(db, "atlassian_endpoint", current_user.id),
        atlassian_secret_set=bool(await _get_setting(db, "atlassian_secret", current_user.id)),
        jira_project_id=await _get_setting(db, "jira_project_id", current_user.id),
        smtp_host=await _get_setting(db, "smtp_host", current_user.id),
        smtp_port=await _get_setting(db, "smtp_port", current_user.id),
        smtp_user=await _get_setting(db, "smtp_user", current_user.id),
        smtp_pass_set=bool(await _get_setting(db, "smtp_pass", current_user.id)),
        smtp_recipient=await _get_setting(db, "smtp_recipient", current_user.id),
    )
...
async def update_settings(data: SettingsUpdate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    for key, value in updates.items():
        await _set_setting(db, key, str(value), current_user.id)
